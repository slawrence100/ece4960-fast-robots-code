...
#define BLE_UUID_TX_IMU_ROLL "6783cd1b-5f31-44e9-8d45-819ba9b9b660"
#define BLE_UUID_TX_IMU_TIMES "ee1c7b18-2a26-4c48-a759-72f86d3178c5"
...
BLEFloatCharacteristic tx_characteristic_float_imu_pitch(BLE_UUID_TX_IMU_PITCH,
                                                    BLERead | BLENotify);
BLEFloatCharacteristic tx_characteristic_float_imu_roll(BLE_UUID_TX_IMU_ROLL,
                                                    BLERead | BLENotify);
BLEFloatCharacteristic tx_characteristic_float_imu_times(BLE_UUID_TX_IMU_TIMES,
                                                    BLERead | BLENotify);
// RX
...
float gyr_keypoints[TOF_HISTORY_LEN];
int tof_front_history_idx = 0;
float pid_motor_power[IMU_HISTORY_LEN];
int pid_motor_power_idx = 0;
int tempMeasurement = -1;

// IMU array has 9 spots:
// [ Accel X, Y, Z | Gyro X Y Z | Mag X Y Z ]
float pitch_history[IMU_HISTORY_LEN];
float roll_history[IMU_HISTORY_LEN];
unsigned long imu_time_history[IMU_HISTORY_LEN];
...
void handle_command() {
  ...
  switch (cmd_type) {
    ...
    case START_DATA_COLLECTION:
      ...
      for (int i = 0; i < IMU_HISTORY_LEN; i++){
        pitch_history[i] = 0.0;
        roll_history[i] = 0.0;
      }
    ...
    case START_PID:
      ...
      if (!success) return;
      success = robot_cmd.get_next_value(pid_setpoint);
      if (!success) return;
    ...
    case GET_IMU_DATA:
      success = robot_cmd.get_next_value(data_idx);
      if (!success) return;
      success = robot_cmd.get_next_value(data_type);
      if (!success) return;
      if (data_type == 1){
        tx_characteristic_float_imu_pitch.writeValue(pitch_history[data_idx]);
      } else if (data_type == 2) {
        tx_characteristic_float_imu_roll.writeValue(roll_history[data_idx]);
      } else if (data_type == 3) {
        tx_characteristic_float_imu_pitch.writeValue(pitch_history[data_idx]);
        tx_characteristic_float_imu_roll.writeValue(roll_history[data_idx]);
      } else {
        tx_characteristic_float_imu_pitch.writeValue(gyr_keypoints[data_idx]);
      }
      tx_characteristic_float_imu_times.writeValue(float(imu_time_history[data_idx] / 1000));
      break;
...
void setup() {
  ...
  testService.addCharacteristic(tx_characteristic_float_imu_roll);
  testService.addCharacteristic(tx_characteristic_float_imu_times);
  ...
  tx_characteristic_float_motor_pid.writeValue(0.0);
  tx_characteristic_float_imu_pitch.writeValue(0.0);
  tx_characteristic_float_imu_roll.writeValue(0.0);
  tx_characteristic_float_imu_times.writeValue(0.0);
...
void get_imu_measurement(ICM_20948_I2C* sensor, bool blocking) {
  ...
  pitch_history[imu_history_idx] = pitch;
  roll_history[imu_history_idx] = roll;
  imu_time_history[imu_history_idx] = micros();
  imu_history_idx++;
}
  ...
  if (central) {
    ...
    while (central.connected()) {
      ...
      if (collect_data) {
        ...
        Serial.print(pitch);
        if (use_pid) {
          // PID the pitch until it's at the angle you want
          // min speed is 170, max speed is 0
          ...
          if (motor_power >= 200 || motor_power <= -200) {
            ...
            spin(-1*motor_power, motor_power * motor_calib_factor);
          } else if (abs(pitch - pid_setpoint) < 0.5) {
